...
taking `null.ok` and little else. `chk_true()` is the catch-all, for any
property that can be written as a condition.
...
the same naming convention. `chk_dots_empty()` fails if anything was
passed through `...`, and `chk_match()` matches an argument against the
values in its own default, returning the match.
